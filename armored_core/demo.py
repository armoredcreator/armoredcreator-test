...
class DemoVision:
    def identify(self, item):
        return VisionResult("finaldomeulinknovo", "https://example.invalid/affiliate")
...
class DemoStudio:
    ...
    def process(self, item):
        ...
        r = self.storage.result(item.item_id, item.affiliate_name or "final")
...
class DemoPublisher:
    ...
    def publish(self, item):
        ...
        self.published[item.item_id] = mid
...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        pub = DemoPublisher()
        Pipeline(db, storage, DemoVision(), DemoStudio(storage), pub).run(item)
        row = db.get(item)
        assert row.state.value == "PUBLISHED"
        assert row.original_path.read_bytes() == b"ORIGINAL"
