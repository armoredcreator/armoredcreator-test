...
class V:
    def identify(self,item): return VisionResult("affiliate-name","https://example.invalid")
class S:
    def __init__(self,s): self.s=s
    def process(self,item):
        w=self.s.working(item.item_id); w.write_bytes(b"WORK")
        r=self.s.result(item.item_id,item.affiliate_name); r.write_bytes(b"RESULT"); return r
class P:
    def __init__(self): self.published=set(); self.calls=0
    def is_published(self,item): return item.item_id in self.published
    def publish(self,item): self.calls+=1; self.published.add(item.item_id); return PublicationResult(True,"telegram-id")

class InvariantTests(unittest.TestCase):
    def test_original_is_never_overwritten(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); st=Storage(root); db=Database(st.database/"db.sqlite")
            src=root/"source.mp4"; src.write_bytes(b"IMMUTABLE")
    ...
    def test_duplicate_ingest_returns_same_item(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); st=Storage(root); db=Database(st.database/"db.sqlite")
            src=root/"source.mp4"; src.write_bytes(b"X"); sync=SyncService(db,st)
            a=sync.ingest(src,"telegram-1"); b=sync.ingest(src,"telegram-1")
