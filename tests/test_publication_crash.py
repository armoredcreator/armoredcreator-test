...
class S:
    ...
    def process(self,item):
        ...
        r=self.s.result(item.item_id,item.affiliate_name); r.write_bytes(w.read_bytes())
        return StudioResult(w, r)

class PublishThenCrash:
    def __init__(self, db):
        self.db=db; self.published=set(); self.calls=0
    def check_publication(self,item):
        if item.item_id in self.published:
            self.db.publication_message_sent(item.item_id, f"telegram-{item.item_id}")
    ...
    def publish(self,item):
        ...
        self.db.publication_message_sent(item.item_id, f"telegram-{item.item_id}")
        raise PublicationUnknownError("crash-after-external-publication")
...
class UnknownPublisher:
    ...
    def publish(self,item):
        self.calls += 1
        raise AssertionError("publish must never run")
    ...
    def test_recovery_does_not_duplicate_after_external_publish(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                publisher=PublishThenCrash(db)
                Pipeline(db,st,V(),S(st),publisher).run(item)
                self.assertEqual(db.get(item).state,State.RECOVERY)
